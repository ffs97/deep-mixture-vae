...
parser.add_argument("--semi_supervised", action="store_true", default=False,
                    help="Whether to use semi-supervised learning or not (ME models)")
parser.add_argument("--num_labels", type=int, default=100,
                    help="Number of labelled samples for semi-supervised setting (ME models)")
...
parser.add_argument("--feature_learning", action="store_true", default=False,
...
def main(argv):
    ...
    feature_learning = argv.feature_learning
    semi_supervised = argv.semi_supervised

    dataset = load_data(
        dataset, classification=classification,
        output_dim=output_dim, labels=moe, ss=semi_supervised
    )
    ...
    if moe:
        ...
        if model_str == "dmoe":
            model = models.DeepMoE(
                model_name, dataset.input_type, dataset.input_dim, dataset.input_shape, output_dim, n_experts, classification,
                activation=tf.nn.relu, initializer=tf.contrib.layers.xavier_initializer, feature_learning=feature_learning
            ).build_graph()
            plotting = False

        elif model_str == "dvmoe":
            model = models.DeepVariationalMoE(
                model_name, dataset.input_type, dataset.input_dim, dataset.input_shape, latent_dim, output_dim, n_experts, classification,
                activation=tf.nn.relu, initializer=tf.contrib.layers.xavier_initializer, feature_learning=feature_learning
            ).build_graph()

        elif model_str == "advmoe":
            model = models.AdversarialDVMoE(
                model_name, dataset.input_type, dataset.input_dim, dataset.input_shape, latent_dim, output_dim, n_experts, classification,
                activation=tf.nn.relu, initializer=tf.contrib.layers.xavier_initializer, feature_learning=feature_learning
            ).build_graph()
        ...
        elif model_str == "admvae":
            model = base_models.AdversarialDMVAE(
                model_name, dataset.input_type, dataset.input_dim, dataset.input_shape, latent_dim, n_clusters,
                lam=dataset.regularization_weight, activation=tf.nn.relu, initializer=tf.contrib.layers.xavier_initializer
            ).build_graph()
        elif model_str == "vade":
    ...
    if pretrain:
        if model_str in ["dvmoe", "vademoe", "advmoe"]:
    ...
    with tqdm(range(n_epochs)) as bar:
        ...
        for epoch in bar:
            ...
            if epoch % save_epochs == 0:
                ...
                acc_ = test_acc if moe else train_acc
            ...
            loss = model.train_op(sess, train_data, kl_ratio=anneal_term)
