...
from includes.digit_struct import DigitStruct
...
def handle_tar_file(file_pointer):
    ...
    for i in range(data_count):
        ...
        if(len(lbls) < MAX_LABELS):
            ...
            print(
                "Skipping {}, only images with less than {} numbers are allowed!".format(
                    file_name, MAX_LABELS
                )
            )
...
def download_data_file(path, filename, force=False):
    ...
    saved_file, _ = urlretrieve(
        base_url + filename, os.path.join(path, filename), reporthook=download_progress)
    print("\nDownload Complete!")
...
def train_validation_spit(train_dataset, train_labels):
    train_dataset, validation_dataset, train_labels, validation_labels = train_test_split(
        train_dataset, train_labels, test_size=0.1, random_state=42)
...
def write_npy_file(data_array, lbl_array, data_set_name, data_path):
    np.save(os.path.join(DATA_PATH+data_path, data_path +
                         "_"+data_set_name+'_imgs.npy'), data_array)
    print('Saving to %s_svhn_imgs.npy file done.' % data_set_name)
    np.save(os.path.join(DATA_PATH+data_path, data_path +
                         "_"+data_set_name+'_labels.npy'), lbl_array)
...
def load_svhn_data(data_type, data_set_name):
    ...
    labels = np.load(os.path.join(
        path, data_set_name+'_'+data_type+'_labels.npy'))
    return imgs, labels
...
def create_label_array(el):
    ...
    for n in range(num_digits):
        if el[n] == 10:
            el[n] = 0  # reassign 0 as 10 for one-hot encoding
...
def create_img_array(file_name, top, left, height, width, out_height, out_width):
    ...
    img = img.crop((box_left, box_top, box_right, box_bottom)).resize(
        [out_height, out_width], Image.ANTIALIAS)
    pix = np.array(img)
